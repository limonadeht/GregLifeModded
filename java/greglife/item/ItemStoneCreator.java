...
public class ItemStoneCreator extends ItemBlock implements IFluidContainerItem{

	public ItemStoneCreator(Block block){
		super(block);
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean bool){
		BlockStoneCreator creator = (BlockStoneCreator)this.field_150939_a;

		FluidTank fakeTankW = readTank(itemStack);
		FluidStack fluidStackW = fakeTankW.getFluid();

		list.add("The Fluid Tanks!");
		if(fluidStackW != null && fluidStackW.amount > 0){
			float percent = Math.max(100.0f / fakeTankW.getCapacity() * fluidStackW.amount / 16, 1);
			list.add(EnumChatFormatting.GREEN + fluidStackW.getLocalizedName() + " " + String.format("%d mB (%.0f%%)", fluidStackW.amount, percent));
		}else{
		...
		if(GregLife.proxy.isShiftKeyDown()){
			list.add("-----------------------");
			list.add("Max Storage: [Water]16000 mb, [Lava]16000 mb");
		}else{
	...
	public static ItemStack createFilledTank(Fluid fluid) {
		final int tankCapacity = TileTank.getTankCapacity();
		...
		ItemStack item = new ItemStack(GLContent.blockTank);
		saveTank(item, tank);
		return item;
	}
	...
	private static FluidTank readTank(ItemStack stack) {
		...
		if (itemTag != null && itemTag.hasKey("LavaTank")) {
			tank.readFromNBT(itemTag.getCompoundTag("LavaTank"));
			return tank;
		}
