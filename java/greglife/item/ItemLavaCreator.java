...
public class ItemLavaCreator extends ItemBlock implements IFluidContainerItem{

	public ItemLavaCreator(Block block){
		super(block);
	}
	...
	public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean bool){
		...
		BlockLavaCreator blockCreator = (BlockLavaCreator)this.field_150939_a;

		FluidTank fakeTank = readTank(stack);
		FluidStack fluidStack = fakeTank.getFluid();

		list.add("The Lava Fabricator!");
		list.add("Stored: " + blockCreator.getEnergyStored(stack) + " / " + blockCreator.getEnergyCapacity());
		...
		if(GregLife.proxy.isShiftKeyDown()){
			list.add("-----------------------");
			list.add("Max Storage: " + blockCreator.getEnergyCapacity());
			list.add("Max Transfer: " + blockCreator.getEnergyTransfer());
		}else{
