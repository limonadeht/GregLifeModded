...
public class ItemSprinkler extends ItemBlock implements IFluidContainerItem{
	...
	public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean bool){
		BlockSprinkler sprinkler = (BlockSprinkler)this.field_150939_a;

		FluidTank fakeTank = readTank(itemStack);
		FluidStack fluidStack = fakeTank.getFluid();

		list.add("The Hyper Growing!");
		list.add("Stored: " + sprinkler.getEnergyStored(itemStack) + " / " + sprinkler.getMaxEnergyStored(itemStack));
		...
		if(GregLife.proxy.isShiftKeyDown()){
			list.add("-----------------------");
			list.add("Max Storage: " + TileSprinkler.getTankCapacity() * 16 + " mb");
		}else{
