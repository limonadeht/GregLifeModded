...
import greglife.gui.inventory.ExclusiveSlot;
import greglife.gui.inventory.SlotTierLocked;
...
public class ContainerGregLifeInfuser extends Container{

	private TileGregLifeInfuser infuser;
	private EntityPlayer player;

	private Slot[] slots = new Slot[11];

	public ContainerGregLifeInfuser(TileGregLifeInfuser infuser, EntityPlayer player, int xSize, int ySize) {
		this.infuser = infuser;
		this.player = player;
		slots[0] = new ExclusiveSlot(infuser, 0, 12, -10).setExclusive(ImmutableSet.of((Item) GLContent.itemUpgradeBook)).setWhitelist(true);
		...
		slots[10] = new ExclusiveSlot(infuser, 10, 80, 33).setExclusive(GregLife.Upgrade_Registry.getTools()).setWhitelist(true);
		layout(xSize, ySize);
	}
	...
	private ImmutableSet<Item> getSetForSlot1() {
		...
		items.add(GLContent.itemLiquidGLBucket);
